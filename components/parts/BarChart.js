...
{data.map(function(datum, index){
            return <Rect onClick={handleChildClick} key={ index } x={x(datum.genre)} y={y(datum.count)} width={x.rangeBand()} height={height - y(datum.count)} genre={datum.genre} count={datum.count} />;
            })}
</g>
	);
},
